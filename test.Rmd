...
coeff=0.5
cumulativeVariance <- ggplot(data = a$pcSum, aes(x = `rowname`, y= a$pcdf$`Cumulative Proportion`)) +
                      geom_line( size = 2, color = a$pcdf$`Cumulative Proportion`) + 
                      geom_line(aes(y = a$pcdf$`Proportion of Variance`), size = 2, color = a$pcdf$`Proportion of Variance`) +
                      scale_y_continuous(
                        # Features of the first axis
                        name = "Cumulative Variance Explained (%)",
                        # Add a second axis and specify its features
                        sec.axis = sec_axis(~.*coeff, name="Propportion of Variance (%)")) + 
                      #theme_ipsum() +
                      theme(axis.title.y = element_text(color = a$pcdf$`Cumulative Proportion`, size=13),
                            axis.title.y.right = element_text(color = a$pcdf$`Proportion of Variance`, size=13)) +
                      ggtitle("Variance") +
                      xlab("PC") 
  
  scale = 0.5

ggplot(a$pcSum, aes(x = `rowname`, y = `Cumulative Proportion`)) +
  geom_line(aes(color = "red")) +
  geom_line(aes(y = `Proportion of Variance`, color = "Cumulative Proportion")) +
  scale_y_continuous(sec.axis = sec_axis(~.*scale, name = "Proportion of Variance (%)")) +
  labs(x = "PC", y = "Cumulative Variance (%)", color = "") +
...
print(pkpd)
  # cumulativeVariance <- ggplot(data = a$pcSum, aes(x = `rowname`)) +
  #   geom_point(aes(y = `Cumulative Proportion`), colour = "blue") +
  #   #geom_line(group = 1, colour = "blue") +
  #   geom_point(aes(y = (`Proportion of Variance`)), colour = "red") +
  #   #geom_line(group = 1, colour = "red") +
  #   ggtitle("Cumulative Variance and Screeplot") +   
  #   theme_bw() +
